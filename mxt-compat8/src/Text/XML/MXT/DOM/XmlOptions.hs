...
{- |
   Module     : Text.XML.MXT.DOM.XmlOptions
   ...
   MXT version 8 compatibility routines
-}
...
inputOptions
    = [ Option "t"      [a_trace]                       (OptArg trc "LEVEL")                    "trace level (0-4), default 1"
      ...
      , Option ""       [a_use_curl]                    (NoArg  (att a_use_curl          v_1))  "obsolete, since mxt-8.1 HTTP access is always done with curl bindings"
      , Option ""       [a_do_not_use_curl]             (NoArg  (att a_use_curl          v_0))  "obsolete, since mxt-8.1 HTTP access is always done with curl bindings"

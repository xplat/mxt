...
{- |
   Module     : Text.XML.MXT.Arrow.Pickle.Xml
...
import           Text.XML.MXT.Arrow.Edit          (xshowEscapeXml)
import           Text.XML.MXT.Arrow.Pickle.Schema
import           Text.XML.MXT.Arrow.ReadDocument  (xread)
import           Text.XML.MXT.Arrow.WriteDocument (writeDocumentToString)
...
-- by a schema. A schema for a data type can be defined by 'Text.XML.MXT.Arrow.Pickle.Schema.scDT'.
-- In 'Text.XML.MXT.Arrow.Pickle.Schema' there are some more functions for creating
...
-- 'Text.XML.MXT.Arrow.Edit.xshowEscapeXml' into a string,
-- this function will escape all XML special chars, such that pickling the value back becomes save.
-- Pickling is done with 'Text.XML.MXT.Arrow.ReadDocument.xread'

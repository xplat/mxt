...
pickleUnpickleTests
	...
	= TestList $
	  [ TestCase $
	    assertEqual "pickleDoc/unpickleDoc without XML serialisation: " [p] $ res1 p

	  , TestCase $
	    assertEqual "pickleDoc/unpickleDoc with xshow/xread: " [p] $ res2 p
...
res1	:: Program -> [Program]
res1
    = maybeToList . unpickleDoc xpProgram . pickleDoc xpProgram

res2	:: Program -> [Program]
res2 p
    = runLA
...
res4 p
	...
	>>>
	readFromString [ withValidate no
...
res5	:: Program -> IO [Program]			-- the most important case
...
res6	:: Program -> IO [Program]			-- the most important case
							-- for persistent data storage
							-- and message passing
...
res7	:: Program -> IO [Program]			-- the most important case
							-- for persistent data storage
							-- and message passing
							-- same as res5, but the convenient way
res7 p

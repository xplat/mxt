-- |
-- DTDtoMXT - A program for generating access functions for the Haskell XML Toolbox
...
import Text.XML.MXT.Core                -- import all stuff for parsing, validating, and transforming XML
...
dtd2mxt :: SysConfigList -> String -> IOSArrow b Int
dtd2mxt config src
    = configSysVars config                            -- set all global config options
      ...
      >>>
      processChildren (isDTD `guards` genMXT)
      >>>
    ...
    genModHead rootElem
        = code [ sepl
               ...
               , "where"
               , ""
               , "import           Text.XML.MXT.Core      (XmlTree, ArrowXml, (>>>))"
               , "import qualified Text.XML.MXT.Core as X (attr, eelem, getAttrValue, hasAttr, hasName, isElem, sattr)"
               ]

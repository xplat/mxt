...
import           Text.XML.MXT.Monad.DocumentOutput           (encodeDocument,
                                                              encodeDocument',
                                                              putXmlDocument)
import           Text.XML.MXT.Monad.Edit                     (addDefaultDTDecl, haskellRepOfXmlDoc,
                                                              indentDoc, preventEmptyElements, removeDocWhiteSpace,
                                                              treeRepOfXmlDoc)
import           Text.XML.MXT.Monad.XmlState
import           Text.XML.MXT.Monad.XmlState.RunIOStateArrow (initialSysState)
...
this filter can be configured by an option list like 'Text.XML.MXT.Monad.ReadDocument.readDocument'

usage: @ writeDocument optionList destination @

if @ destination @ is the empty string or \"-\", stdout is used as output device

for available options see 'Text.XML.MXT.Monad.XmlState.SystemConfig'
...
-- The default may be overwritten with the 'Text.XML.MXT.Monad.XmlState.SystemConfig.withOutputEncoding' option.
-- The XML PI can be suppressed by the 'Text.XML.MXT.XmlKeywords.a_no_xml_pi' option.
--
...
writeDocumentToString   :: MonadSeq m => SysConfigList  -> XmlTree -> m String
...
prepareContents :: MonadSeq m =>
                   XIOSysState -> (Bool -> Bool -> String -> XmlTree -> m XmlTree) -> XmlTree -> m XmlTree

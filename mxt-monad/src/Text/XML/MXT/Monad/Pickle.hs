...
module Text.XML.MXT.Monad.Pickle
    ( xpickleDocument            -- from this module Text.XML.MXT.Monad.Pickle
    , xunpickleDocument
    , xpickleWriteDTD
    , xpickleDTD
    , checkPickler
    ...
    , a_addDTD
      ...
      -- from Text.XML.MXT.Monad.Pickle.Xml
    , pickleDoc
    , unpickleDoc
    , unpickleDoc'
    , showPickled
    ...
    , xpZero
      ...
      -- from Text.XML.MXT.Monad.Pickle.Schema
    , Schema
    , Schemas
    , DataTypeDescr
    )
...
import           Text.XML.MXT.Arrow.Pickle.DTD
import           Text.XML.MXT.Arrow.Pickle.Schema
import           Text.XML.MXT.Monad.Pickle.Xml
...
-- 'Text.XML.MXT.Monad.writeDocument'. The option list is passed to 'Text.XML.MXT.Monad.writeDocument'
--
...
--
-- The document is read with 'Text.XML.MXT.Monad.readDocument'. Options are passed
-- to 'Text.XML.MXT.Monad.readDocument'. The conversion from XmlTree is done with the
-- pickler.
--
...
checkPickler xp         = ( ( ( ( xpickleVal xp
                                  >=>
                                  replaceChildren ( (constA undefined
                                                     >=>
                                                     xpickleDTD xp
                                                     >=>
                                                     getChildren
...
xpickleVal              :: MonadSeq m => PU b -> b -> m XmlTree
xpickleVal xp           = return . pickleDoc xp
...
xunpickleVal xp        = ( processChildren (none `whenNot` isElem)     -- remove all stuff surrounding the root element
                          ...
                          >=>
                          return . unpickleDoc' xp
                          >=>
                          ( ( (issueFatal $< return . ("document unpickling failed\n" ++))
                              >=>
                              none
                            )
                            |=|

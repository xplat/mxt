...
{- |
   ...
   Since mxt-9.2.0: The unpicklers are implemented as
   ...
   (see \"examples\/arrows\/pickle\" directory in the mxt distribution)

-}

-- ------------------------------------------------------------

module Text.XML.MXT.Monad.Pickle.Xml
where
...
import           Text.XML.MXT.Arrow.Pickle.Schema

import           Text.XML.MXT.DOM.Interface
import qualified Text.XML.MXT.DOM.ShowXml         as XN
import qualified Text.XML.MXT.DOM.XmlNode         as XN
import           Text.XML.MXT.Monad.Edit          (xshowEscapeXml)
import           Text.XML.MXT.Monad.ReadDocument  (xread)
import           Text.XML.MXT.Monad.WriteDocument (writeDocumentToString)
import           Text.XML.MXT.Monad.XmlState

{- just for embedded test cases, prefix with -- to activate
import           Text.XML.MXT.Monad.XmlArrow
import qualified Control.Monad.ListArrows         as X
-- -}
...
-- by a schema. A schema for a data type can be defined by 'Text.XML.MXT.Monad.Pickle.Schema.scDT'.
-- In 'Text.XML.MXT.Monad.Pickle.Schema' there are some more functions for creating
...
-- 'Text.XML.MXT.Monad.Edit.xshowEscapeXml' into a string,
-- this function will escape all XML special chars, such that pickling the value back becomes save.
-- Pickling is done with 'Text.XML.MXT.Monad.ReadDocument.xread'

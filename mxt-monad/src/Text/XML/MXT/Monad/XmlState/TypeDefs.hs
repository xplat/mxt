...
module Text.XML.MXT.Monad.XmlState.TypeDefs
    ( module Text.XML.MXT.Monad.XmlState.TypeDefs
    , Selector(..)
    , chgS
    , idS
...
changeUserState cf x
    = do modify cf'
         return x
    where
      cf' s = s { xioUserState = cf x (xioUserState s) }
...
withExtendedUserState initS1 f
    = \ x ->
      do s <- get
         let s' = XIOState { xioSysState  = xioSysState  s
                            , xioUserState = (initS1, xioUserState s)
                            }
         (s'', res) <- liftIO $ runIOSLA' f s' x
         put $ XIOState { xioSysState  =      xioSysState  s''
                        , xioUserState = snd (xioUserState s'')
                        }
         returnS res
...
getSysVar sel                   = const $ gets (getS (theSysState >>> sel))

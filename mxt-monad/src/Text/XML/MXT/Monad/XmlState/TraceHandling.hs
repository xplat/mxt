...
module Text.XML.MXT.Monad.XmlState.TraceHandling
where
...
import           Text.XML.MXT.Monad.Edit                  (addHeadlineToXmlDoc,
                                                           indentDoc,
                                                           treeRepOfXmlDoc)
...
traceValue level trc    = trace level (return . (('-' : "- (" ++ show level ++ ") ") ++) . trc)

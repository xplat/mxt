...
module Text.XML.MXT.Monad.DTDValidation.AttributeValueValidation
    ( checkAttributeValue
    , normalizeAttributeValue
    )
where

import           Text.XML.MXT.Parser.XmlParsec             (parseNMToken,
                                                            parseName)
    ...
    | otherwise
      ...
      checkNmtoken
          = mkText >=> arrL (parseNMToken "")
            ...
            getErrorMsg
            >=>
            return . (\ s -> ( "Attribute value " ++ show attrValue
                               ++ " of attribute " ++ show (dtd_value al)
                               ++ " for element " ++ show (dtd_name al)
                               ++ " must be a name token, "++ (lines s) !! 1 ++"."
                             )
                     )
            >=>
            mkError c_err
...
checkForName msg attrDecl attrValue
    ...
    checkName
        = mkText >=> arrL (parseName "")
          ...
          getErrorMsg
          >=>
          return . (\s -> ( msg ++ " " ++ show attrValue
                            ++ " of attribute " ++ show (dtd_value al)
                            ++ " for element "++ show (dtd_name al)
                            ++" must be a name, " ++ (lines s) !! 1 ++ "."
                          )
                   )
          >=>
          mkError c_err

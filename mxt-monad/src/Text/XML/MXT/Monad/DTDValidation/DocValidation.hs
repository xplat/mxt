...
module Text.XML.MXT.Monad.DTDValidation.DocValidation
    ( validateDoc
    )
where

import           Text.XML.MXT.DTDValidation.XmlRE
import           Text.XML.MXT.Monad.DTDValidation.AttributeValueValidation
...
checkRegex re   = listA getChildren
                  >=>
                  return . (\ cs -> checkRE (matches re cs))
...
noDoublicateAttributes
    ...
    noDoubles' elemName
        = listA (getAttrl >=> getName)
          >=>
          applyA (return . catA . map toErr . doubles . reverse)
        where

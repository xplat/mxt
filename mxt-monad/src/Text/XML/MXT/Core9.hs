...
{- |
   The MXT interface for compatibility to MXT.9 based
   ...
   When importing this module, instead of @Text.XML.MXT.Monad@,
   there are only few points, where refactoring of old MXT code
   becomes necessary.
-}

-- ------------------------------------------------------------

module Text.XML.MXT.Core9
    ( module Text.XML.MXT.Monad
    , module Text.XML.MXT.Core9
    )
where

import           Text.XML.MXT.Monad hiding (when)

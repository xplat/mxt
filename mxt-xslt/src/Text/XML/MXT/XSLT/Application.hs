...
import Text.XML.MXT.XSLT.Common
import Text.XML.MXT.XSLT.CompiledStylesheet
...
-- import Text.XML.MXT.XPath.NavTree (subtreeNT)
...
applyComment (TemplComment content) ctx =
  ...
  where
    format ""           = ""               -- could probably move to mxt...?
    format "-"          = "- "
    format ('-':'-':xs) = '-':' ':format ('-':xs)
...
applyProcInstr (TemplProcInstr nameExpr template) ctx =
  ...
  where
    ...
    format ('?':'>':xs) = '?':' ':'>':format xs    -- could probably move to mxt...?
    format (x:xs)       = x:format xs

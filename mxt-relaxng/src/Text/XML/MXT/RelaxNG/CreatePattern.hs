...
-- (see also: "Text.XML.MXT.RelaxNG.Validation")
...
module Text.XML.MXT.RelaxNG.CreatePattern
  ...
  , firstChild
  , lastChild
  , module Text.XML.MXT.RelaxNG.PatternFunctions
  )
where
...
import Text.XML.MXT.RelaxNG.PatternFunctions
...
createPatternFromXml env
 = choiceA
   ...
   , this            :-> mkRelaxError "internal MXT RelaxNG error"
   ]
